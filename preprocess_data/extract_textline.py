...
from extractor.utils import order_points, to_4points, to_quad_fast, to_quad_slow
...
def get_text_image(coord, image, pad_ratio):
...
rotator_obj = rotator.LabelRotator(
    ignore_wrong_text=False, device="cpu", text_ratio=5, hor_text_ratio=5
)
...
extractor_obj = extractor.LabelExtractor(
    regions=regions, to_quad=True, to_crop=False, pad_ratio=0.02, is_fast=True
)
...
if config["process_only"]:
    data_subset = open(config["process_only"]).readlines()
    process_files = [line.strip().split("\t") for line in data_subset]
else:
    process_files = Path(config["raw_data_path"]).rglob(
        f"{config['DIR_PREFIX']}*/*.json"
    )
...
for f_ in process_files:
    ...
    with open(str(f_), encoding="utf-8") as f:
        anno = json.load(f)

    try:
        extracted_imgs, extracted_json_infos, region_names = extractor_obj.extract(
            img, anno
        )
    ...
    for extracted_img, extracted_json_info, region_name in zip(
        extracted_imgs, extracted_json_infos, region_names
    ):
        new_shape = []
        field_by_region = config["regions"][region_name]
        exclude_field_by_region, include_field_by_region = [], []

        for field_name in field_by_region:
            if field_name.startswith("^"):
                exclude_field_by_region.append(field_name[1:])
            else:
                include_field_by_region.append(field_name)

        for box in extracted_json_info["shapes"]:
            if box["label"] in regions:
                continue

            if len(exclude_field_by_region) > 0 and any(
                [box["label"].startswith(field) for field in exclude_field_by_region]
            ):
                continue
            if len(include_field_by_region) > 0 and not all(
                [box["label"].startswith(field) for field in include_field_by_region]
            ):
                continue
            ...
            if box["shape_type"] == "rectangle":
                ...
                points = to_quad_fast(
                    points, extracted_img.shape[0], extracted_img.shape[1]
                )
                ...
                for point1, point2 in combinations(points, 2):
                    if (
                        np.abs(point1[0] - point2[0]) < 3
                        and np.abs(point1[1] - point2[1]) < 3
                    ):
                        collapse_point = point1
                        remove_point = point2
                        break

                new_point = [collapse_point] + [
                    point
                    for idx, point in enumerate(points)
                    if point[0] != remove_point[0] and point[1] != remove_point[1]
                ]
                points = order_points(new_point)

            if box["shape_type"] in ["polygon", "rectangle"] and len(points) == 4:
        ...
        rotated_img, rotated_json_info = rotator_obj.rotate(
            extracted_img, extracted_json_info
        )
        ...
        for box in boxes:
            ...
            if config["extract_img_only"]:
                label = box["label"]
                label_dir = save_dir.joinpath(region_name).joinpath(box["label"])
                if not label_dir.exists():
                    label_dir.mkdir(parents=True)

                img_save_path = str(label_dir.joinpath(save_img_name))
                points = [tuple(point) for point in box["points"]]
                tl_img = get_text_image(
                    points, rotated_img, pad_ratio=config["pad_info"]
                )
            ...
            if value is None:
                ...
                img_save_path = str(label_dir.joinpath(save_img_name))
                label_save_path = str(
                    label_dir.joinpath(save_img_name).with_suffix(".txt")
                )
