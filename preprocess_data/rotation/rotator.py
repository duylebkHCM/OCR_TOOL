...
class LabelRotator:
    def __init__(
        ...
        text_height: int = 32,
        hor_text_ratio: float = 2.0,  # text_width / text_height
        max_texts: int = 10,
        ...
        self.ratio_croper = iaa.CropToAspectRatio(
            text_ratio, position="right-bottom"
        )  # crop text image to fixed ratio
        self.ratio_padder = iaa.PadToAspectRatio(
            text_ratio, position="right-bottom"
        )  # pad text image to fixed ratio
    ...
    def rotate(self, image: np.ndarray, json_info: dict) -> Tuple[np.ndarray, dict]:
        ...
        for shape in json_info["shapes"]:
            ...
            shape["points"] = [
                [keypoint.x, keypoint.y] for keypoint in kps_on_image.keypoints
            ]

        image = rotator(image=image)

        if angle % 180:
            json_info["imageWidth"], json_info["imageHeight"] = (
                json_info["imageHeight"],
                json_info["imageWidth"],
            )
    ...
    def get_angle_document(self, image: np.ndarray, json_info: dict) -> int:
        ...
        if (not len(hor_texts)) and (
            not len(ver_texts)
        ):  # angle = 0 if image has no horizontal texts and vertical texts
            return 0

        if len(hor_texts) > len(ver_texts):
            texts = sorted(
                hor_texts,
                key=lambda text: self.distance(text[0], text[1]),
                reverse=True,
            )
            angle = 0
        else:
            texts = sorted(
                ver_texts,
                key=lambda text: self.distance(text[0], text[3]),
                reverse=True,
            )
            angle = 90
        ...
        if len(texts) > self.max_texts:
            texts = texts[
                (len(texts) - self.max_texts) // 2 : (len(texts) + self.max_texts) // 2
            ]
        ...
        for points in texts:
            w = int(
                round(
                    (
                        self.distance(points[0], points[1])
                        + self.distance(points[3], points[2])
                    )
                    / 2
                )
            )
            h = int(
                round(
                    (
                        self.distance(points[0], points[3])
                        + self.distance(points[1], points[2])
                    )
                    / 2
                )
            )
            ...
            if w / (h + 1e-6) > self.text_ratio:
                ...
                text = (
                    (text - text.mean()) / text.std()
                    if text.std()
                    else np.zeros_like(text)
                )
            else:
                text = (
                    (text - text.mean()) / text.std()
            ...
            text = cv2.resize(
                text,
                dsize=(
                    int(round(self.text_height * self.text_ratio)),
                    self.text_height,
                ),
            )
        ...
        if (
            preds.round().sum() / (preds.numel() + 1e-6) > 0.5
        ):  # voting (0: 0 degree, 1: 180 degree)
            angle += 180
            ...
            if shape["shape_type"] == "polygon":
                ...
                if len(points) != 4:  # not a 4 points polygon (not a true text line)
                    if self.ignore_wrong_text:
                        ...
                        raise RuntimeError(
                            f"{json_info['imagePath']} with {shape['label']} must be 4 points"
                        )
    ...
    def compute_text_ratio(self, points: List[Tuple[float, float]]) -> float:
        w = (
            self.distance(points[0], points[1]) + self.distance(points[3], points[2])
        ) / 2
        h = (
            self.distance(points[0], points[3]) + self.distance(points[1], points[2])
        ) / 2
        return w / (h + 1e-6)
