...
from .utils import to_quad_slow, to_quad_fast, to_crop, to_warp, to_4points, to_perspective, centroid_poly
...
class LabelExtractor:
    def __init__(
        self,
        regions: List[str],
        pad_ratio: Union[List[float], float] = [0., 0., 0., 0.],  # left, right, top, bottom
        is_fast: bool = True,
        to_quad: bool = True,
        to_crop: bool = False,
    ) -> None:
    ...
    def extract(
        ...
        warped_images, warped_persps, warped_locs, region_names = self.get_warped_regions(image, json_info)

        for idx, (warped_image, warped_persp, warped_loc) in enumerate(zip(warped_images, warped_persps, warped_locs)):
            warped_json_info = {}
            warped_json_info['version'] = json_info.get('version', '4.5.7')
            warped_json_info['flags'] = json_info.get('flags', {})
            warped_json_info['imagePath'] = f"{Path(json_info['imagePath']).stem}_{idx}{Path(json_info['imagePath']).suffix}"
            ...
            for shape in json_info['shapes']:
                ...
                if warped_loc.contains(Point(center_point)) and warped_loc.area > Polygon(points).area:
                ...
                if (shape['label'] in self.regions) and (
                    shape['label'] not in [warped_shape['label'] for warped_shape in warped_json_info['shapes']]
                ):
                    location = Polygon(points.tolist()).buffer(0)
                    if warped_loc.intersection(location).area / warped_loc.union(location).area > 0.9:
                        warped_shape = copy.deepcopy(shape)
                        warped_shape['points'] = points.tolist()
                        warped_shape['shape_type'] = shape_type
                        warped_json_info['shapes'].append(warped_shape)
    ...
    def get_warped_regions(self, image: np.ndarray, json_info: Dict) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        ...
        for info in json_info['shapes']:
            region_name, region_coords, region_type = info['label'], info['points'], info['shape_type']
            ...
            if (region_name in self.regions) and (len(region_coords) >= 4):
                if self.to_quad:
                    ...
                    raise ValueError('Must be choose warp method is to_quad or to_crop.')

                if warped_coords is not None:
                    warped_image, warped_persp = to_warp(image=image, points=warped_coords)
