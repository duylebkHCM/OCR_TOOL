...
from text_renderer.config import GenerateCfg, import_module_from_file
...
class WriterProcess(Process):
    def __init__(self, data_queue, save_dir: Path, jpg_quality: int = 95):
        super().__init__()
...
def generate_img(data_queue, idx, save_name):
    ...
    if data_queue is not None and data[0] is not None:
...
def parse_args():
    ...
    parser.add_argument(
        "--config", default="example/config.py", help="python config file path"
    )
    parser.add_argument(
        "--effects",
        nargs="+",
        default=["dropout_rand_padding_blur"],
        help="Choose which effects in the config that will be used",
    )
    parser.add_argument(
        "--buffer",
        default=1000,
        help="Maximum number of sample will by save into a sub output directory",
    )
